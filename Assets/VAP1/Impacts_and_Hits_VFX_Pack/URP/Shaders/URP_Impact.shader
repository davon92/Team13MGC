	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				float4 saferPower27 = abs( ( ( IN.ase_color * tex2DNode5 ) * _Emission_Strength ) );
				float4 temp_cast_0 = (_Emission_Contrast).xxxx;
				
				float Erosion_X32 = IN.ase_texcoord4.x;
				...
				float Erosion_Noise_Y33 = IN.ase_texcoord4.y;
				...
				float3 Color = pow( saferPower27 , temp_cast_0 ).rgb;
				float Alpha = ( IN.ase_color.a * saturate( pow( saferPower19 , temp_cast_5 ) ) ).r;
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float Erosion_X32 = IN.ase_texcoord2.x;
				float4 temp_cast_0 = (Erosion_X32).xxxx;
				float4 temp_cast_1 = (1.0).xxxx;
				float2 uv_MainTexture = IN.ase_texcoord3.xy * _MainTexture_ST.xy + _MainTexture_ST.zw;
				float4 tex2DNode5 = tex2D( _MainTexture, uv_MainTexture );
				float4 smoothstepResult7 = smoothstep( temp_cast_0 , temp_cast_1 , tex2DNode5);
				float2 uv_Noise = IN.ase_texcoord3.xy * _Noise_ST.xy + _Noise_ST.zw;
				float2 panner15 = ( 1.0 * _Time.y * _Noise_Speed + uv_Noise);
				float Erosion_Noise_Y33 = IN.ase_texcoord2.y;
				float4 temp_cast_2 = (Erosion_Noise_Y33).xxxx;
				float4 saferPower19 = abs( ( ( smoothstepResult7 * saturate( ( tex2D( _Noise, panner15 ) - temp_cast_2 ) ) ) * _Alpha_Strength ) );
				float4 temp_cast_3 = (_Alpha_Contrast).xxxx;
				
				float Alpha = ( IN.ase_color.a * saturate( pow( saferPower19 , temp_cast_3 ) ) ).r;
				float AlphaClipThreshold = 0.5;
...
/*ASEBEGIN
Version=18935
0;0;1920;1059;1082.563;605.252;1;True;True
...
ASEEND*/
//CHKSM=30191113E30E3F5C2BDF6DF8C587D478B964B99C